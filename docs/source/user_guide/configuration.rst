...
.. list-table::
   ...
   * - ``properties_list``
     - Mapping from ALF property names to HDF5 dataset names, property scope,
       and unit conversion factors. Supported scopes are ``system`` (one value
       per structure), ``atomic`` (``[atoms, 3]``), and ``pair_atomic``
       (``[pairs, atoms, 3]``, used for nonadiabatic coupling). See
       :doc:`units` for the conversion direction and common factors.
   * - ``parsl_configuration``
