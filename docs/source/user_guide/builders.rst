...
ordering. ASE's CFG reader marks loaded structures as periodic. Molecular
workflows can set ``"pbc": false`` in the builder configuration to override
that reader default; omitting the field preserves the existing periodic
behavior. CFG supplies geometry, species, cell, and atom order only in this
workflow. Multi-state labels are generated by a QM task or read from ALF HDF5
shards rather than CFG auxiliary fields.
