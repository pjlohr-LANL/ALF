...
if _alchemi_model_import_error is None:
    ...
    class ALFExcitedStateAlchemiModel(torch.nn.Module, BaseModelMixin):
        ...
        def _predict(self, species, positions):
            kwargs = {
                self.species_key: species.reshape(1, -1).to(torch.long),
                self.coordinates_key: positions.reshape(1, -1, 3),
            }
        ...
        def forward(self, batch):
            ...
            predictions = self._predict(species, positions)

            if isinstance(predictions, (tuple, list)):
                raise RuntimeError("ALCHEMI BAOAB Hippynn adapter requires dict-like Predictor outputs.")
            else:
                extra_values = [
                    _prediction_value(predictions, node, key)
                    for key, node in self.extra_properties.items()
                ]

            if self._mode.get("name") == "lcm_gap":
                lower_energy = _prediction_value(predictions, self._mode["lower_energy_node"], "lower_energy")
                upper_energy = _prediction_value(predictions, self._mode["upper_energy_node"], "upper_energy")
                lower_forces = _prediction_value(predictions, self._mode["lower_force_node"], "lower_forces")
                upper_forces = _prediction_value(predictions, self._mode["upper_force_node"], "upper_forces")
                if self._mode["gap_node"] is None:
                    gap = upper_energy - lower_energy
                else:
                    gap = _prediction_value(predictions, self._mode["gap_node"], "gap")
                ...
                lower_forces = lower_forces.reshape_as(positions)
                upper_forces = upper_forces.reshape_as(positions)
                forces = 0.5 * (lower_forces + upper_forces) + gap_derivative.reshape(-1)[0] * (
                    upper_forces - lower_forces
                )
            else:
                energy = _prediction_value(predictions, self.energy_node, "energy")
                if self.force_node is None:
                    ...
                    forces = _prediction_value(predictions, self.force_node, "forces").reshape_as(positions)

            energy = energy.reshape(-1).sum() + torch.as_tensor(
                self.offset_eV,
...
def build_alchemi_batch(atoms, *, device, dtype=None):
    ...
    _batch_set(batch, "positions", positions)
    _batch_set(batch, "atomic_numbers", atomic_numbers)
    _batch_set(batch, "atomic_masses", masses)
    _batch_set(batch, "velocities", velocities)
    return batch
...
class AlchemiBaoabRunner:
    ...
    def set_temperature(self, *, temperature_K: float) -> None:
        ...
        if hasattr(self.dynamics, "_temperature_init"):
            self.dynamics._temperature_init = value
    ...
    def sync_to_atoms(self, atoms) -> None:
        positions = _tensor_to_numpy(_batch_get(self.batch, "positions"))
        atoms.set_positions(np.asarray(positions, dtype=float))
        try:
            velocities = _tensor_to_numpy(_batch_get(self.batch, "velocities"))
    ...
    def kinetic_energy_eV(self) -> float:
        ...
        energy = 0.5 * torch.sum(masses * velocities * velocities)
        return float(energy.detach().cpu().item())

    def temperature_K(self) -> float:
        masses = _as_tensor(_batch_get(self.batch, "atomic_masses")).reshape(-1, 1)
        dof = max(1, int(3 * masses.shape[0]))
        return float((2.0 * self.kinetic_energy_eV()) / (dof * KB_EV_PER_K))
