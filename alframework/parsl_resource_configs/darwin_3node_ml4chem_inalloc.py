...
def _allocation_nodes() -> list[str]:
    ...
    if len(nodes) < 3:
        raise RuntimeError(
            "darwin_3node_ml4chem_inalloc requires at least 3 allocated nodes. "
...
def _srun_overrides(node_name: str) -> str:
    return (
        f"--nodes=1 --ntasks=1 --ntasks-per-node=1 --nodelist={node_name} "
        "--cpu-bind=none"
    )


class DarwinInAllocationSrunLauncher:
    """Srun launcher that avoids Parsl's shared cmd_$SLURM_JOB_NAME.sh race."""
    ...
    executors=[
        HighThroughputExecutor(
            ...
            provider=LocalProvider(
                ...
                launcher=DarwinInAllocationSrunLauncher(overrides=_srun_overrides(_node_qm)),
            ),
        ...
        HighThroughputExecutor(
            ...
            provider=LocalProvider(
                ...
                max_blocks=1,
                worker_init=_worker_init_string,
                launcher=DarwinInAllocationSrunLauncher(overrides=_srun_overrides(_node_master)),
            ),
            max_workers_per_node=4,
            available_accelerators=4,
            cores_per_worker=1.0,
